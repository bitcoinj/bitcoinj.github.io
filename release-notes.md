...
* JDK 11+ is required for building the project. In particular, we support LTS releases JDK 11, 17 and 21.
...
* All usages of Guava `ListenableFuture` have been migrated to `CompletableFuture`. Make sure in the 0.17 cycle, 
  migrate all usage of Guava `ListenableFuture` to Java 8 `CompletableFuture`. The former will be removed after 0.17.
  To ease the migration, we use `ListenableCompletableFuture` which implements both. 
...
* If you've been using `PeerDataEventListener` to observe the blockchain download, use `BlockchainDownloadEventListener`
  now.
* Various fields have been made private or deprecated in our quest to make more classes immutable
* protobuf:
  * moved protobufs to `org.bitcoinj.protobuf.*` packages
  * wallet.proto: rename `ENCRYPTED_SCRYPT_AES` to just `ENCRYPTED`
...
* BlockFileLoader: cannot be used directly as `Iterator` any more, call `.iterator()` first
...
* many APIs that used `NetworkParameters` now use `Network`; we're providing deprecation stubs where possible* We're on a mission to replace Guava with JDK 7+ equivalents. Our new `org.bitcoinj.base` package is not depending
  on Guava at all!
